...
#Topic Modeling angry
library(devtools)
library(TextWiller)
library(stopwords)

rec_angry=reviews[reviews$App=="Angry Birds Classic",2]
...
rec_angry=gsub("fack","fuck",rec_angry)
rec_angry=gsub("angri","angry",rec_angry)
rec_angry=gsub("adds","ads",rec_angry)
angry_corpus = Corpus(VectorSource(rec_angry))

angry_corpus = tm_map(angry_corpus, content_transformer(tolower))
angry_corpus = tm_map(angry_corpus, removeNumbers)
angry_corpus = tm_map(angry_corpus, removePunctuation)
stopwordsGrande=stopwords::stopwords("en",source="stopwords-iso")
length(stopwordsGrande)
angry_corpus = tm_map(angry_corpus, removeWords, c("the", "and", "aah", "chorus","echo", "oh", "yo", "yeah", "ha ha", 
                                                   "just","dont","like","ohyea","im","aint","youre","gonna",
                                                   "ah ah","wanna",stopwordsGrande,stopwords("english")))
angry_corpus =  tm_map(angry_corpus, stripWhitespace)

inspect(angry_corpus[1:3])

require(tau)
require(lda)
library(topicmodels)

bigrams <- textcnt(angry_corpus,method="string",n=2L,split="[[:blank:]]")
sort(bigrams,decreasing=TRUE)[1:20]

trigrams <- textcnt(angry_corpus,method="string",n=3L,split="[[:blank:]]")
sort(trigrams,decreasing=TRUE)[1:10]

dtm=DocumentTermMatrix(angry_corpus,
...
q_lda <- LDA(dtm, k = 5, control = list(seed = 112358))
q_lda
...
angry_corpus = tm_map(angry_corpus, removeWords, c("the", "and", "aah", "chorus","echo", "oh", "yo", "yeah", "ha ha", 
                                                   ...
                                                   "ah ah","wanna",stopwordsGrande,stopwords("english")))
angry_corpus =  tm_map(angry_corpus, stripWhitespace)
angry_corpus=as.vector(angry_corpus)
# applico il sentiment (vocabolario personalizzato italiano)
angry_sentiment=sentiment(angry_corpus$content)
...
prop.table(table(angry_sentiment))  # % di tweet tra negativi, neutri, positivi
barplot(table(angry_sentiment))
